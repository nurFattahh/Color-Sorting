...
void loop() {
  ...
  for (int S1value = 200; S1value >= 500; S1value++) {
    srituhobby.setPWM(servo1, 0, S1value);
    delay(10);
  }

  for (int S2value = 150; S2value <= 500; S2value++) {
    srituhobby.setPWM(servo2, 0, S2value);
    delay(10);
  }

  for (int S3value = 150; S3value <= 550; S3value++) { //MAKS 550
    srituhobby.setPWM(servo3, 0, S3value);
    delay(10);
  }

  for (int S4value = 410; S4value <= 550; S4value++) {
